...
# ── Delta logic ────────────────────────────────────────────────────────────────
# Moved to store-driven diffing in run()
...
def run() -> None:
    ...
    # ── Step 2: Connect to Gemini & Fetch Store State ─────────────
    try:
        client = uploader.get_client()
        store = uploader.get_or_create_store(client)
        store_index = uploader.get_store_index(client, store.name)
    except Exception as exc:
        logger.error("Cannot connect to Gemini or fetch store index: %s", exc)
        sys.exit(0)

    # ── Step 3: Compute delta ─────────────────────────────────────
    new_files: list[tuple[Path, str, str]] = []
    changed_files: list[tuple[Path, str, str, str]] = []  # path, display_name, hash8, old_doc_name
    ...
    for path in article_paths:
        ...
        display_name = f"{slug}__{hash8}.md"

        if slug not in store_index:
            new_files.append((path, display_name, hash8))
        elif store_index[slug]["hash8"] != hash8:
            old_doc_name = store_index[slug]["document_name"]
            changed_files.append((path, display_name, hash8, old_doc_name))
        else:
    ...
    for path, display_name, hash8 in new_files:
        doc = uploader.upload_document(client, store.name, path, display_name=display_name)
        if doc:
            added += 1
            logger.info("ADDED: %s as %s", path.name, display_name)
        else:
            failed += 1

    # ── Step 5: Process changed articles ─────────────────────────
    for path, display_name, hash8, old_doc_name in changed_files:
        ...
        if doc:
            updated += 1
            logger.info("UPDATED: %s as %s", path.name, display_name)
        else:
            failed += 1
